...
INSTANCE_META_FOLDER = "instance_meta"
INSTANCE_EXPLORATION_META_FOLDER = "exploration_instance_meta"
...
def save_frame(env, event, root_dir, task_desc='None', folder_name="OBJECT_META_FOLDER"):
    # META DATA
    agent_meta_path = os.path.join(root_dir, INSTANCE_META_FOLDER)
    # EXPLORATION_IMG
    if folder_name == "EXPLORATION":
        agent_meta_path = os.path.join(root_dir, INSTANCE_EXPLORATION_META_FOLDER)
    # META DATA
    ...
    meta_file = os.path.join(agent_meta_path, "%09d.json" % im_idx)
...
def augment_traj(env, json_file):
    ...
    instance_meta_path = os.path.join(root_dir, INSTANCE_META_FOLDER)
    exploration_instance_meta_path = os.path.join(root_dir, INSTANCE_EXPLORATION_META_FOLDER)

    # fresh images list
    traj_data['images'] = list()

    clear_and_create_dir(instance_meta_path)
    clear_and_create_dir(exploration_instance_meta_path)
    ...
    if args.smooth_nav and args.time_delays:
        ...
        object_meta_count = get_json_index(instance_meta_path)
