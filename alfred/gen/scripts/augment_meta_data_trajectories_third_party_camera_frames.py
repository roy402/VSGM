...
THIRDPARTYCAMERAS_OBJECT_META_FOLDER = ["sgg_meta_1", "sgg_meta_2"]
THIRDPARTYCAMERAS_EXPLORATION_META_FOLDER = ["exploration_meta_1", "exploration_meta_2"]
...
def save_frame(env, event, root_dir, task_desc='None', folder_name="OBJECT_META_FOLDER"):
    # META DATA
    thirdPartyCameras = [os.path.join(root_dir, THIRDPARTYCAMERAS_OBJECT_META_FOLDER[0]), os.path.join(root_dir, THIRDPARTYCAMERAS_OBJECT_META_FOLDER[1])]
    # EXPLORATION_IMG
    if folder_name == "EXPLORATION":
        thirdPartyCameras = [os.path.join(root_dir, THIRDPARTYCAMERAS_EXPLORATION_META_FOLDER[0]), os.path.join(root_dir, THIRDPARTYCAMERAS_EXPLORATION_META_FOLDER[1])]
        for third_id, dict_image in enumerate(event.third_party_camera_frames):
            ...
            image = dict_image["frame"][:, :, ::-1]
            cv2.imwrite(path + '/%09d.png' % (im_ind), image)
    for third_id, dict_image in enumerate(event.third_party_camera_frames):
        meta_path = thirdPartyCameras[third_id]
        im_idx = get_json_index(meta_path)
        all_objects = dict_image["objects"]
        sgg_meta_file = os.path.join(meta_path, "%09d.json" % (im_idx))
...
def augment_traj(env, json_file):
    ...
    thirdparty_object_meta_dir_1 = os.path.join(root_dir, THIRDPARTYCAMERAS_OBJECT_META_FOLDER[0])
    thirdparty_object_meta_dir_2 = os.path.join(root_dir, THIRDPARTYCAMERAS_OBJECT_META_FOLDER[1])
    thirdparty_exploration_dir_1 = os.path.join(root_dir, THIRDPARTYCAMERAS_EXPLORATION_META_FOLDER[0])
    thirdparty_exploration_dir_2 = os.path.join(root_dir, THIRDPARTYCAMERAS_EXPLORATION_META_FOLDER[1])

    # fresh images list
    traj_data['images'] = list()

    clear_and_create_dir(thirdparty_object_meta_dir_1)
    clear_and_create_dir(thirdparty_object_meta_dir_2)
    clear_and_create_dir(thirdparty_exploration_dir_1)
    clear_and_create_dir(thirdparty_exploration_dir_2)
    ...
    print(thirdparty_object_meta_dir_1)
